<?php
if (isset($_POST["signup-eleman-submit"])) {
    ...
    if ( empty($name) || empty($surname) ||empty($username) || empty($email) || empty($password) || empty($passwordRepeat) || empty($_FILES["file"]["name"])) {
    ...
    //Allow max size
        ...
        if( !mysqli_stmt_prepare($stmt, $sql)){
            ...
            if( $resultcheck > 0){
                ...
                $sql = "INSERT INTO users (imageUsers, nameUsers, surnameUsers, uidUsers, emailUsers, pwdUsers, typeUsers) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if( !mysqli_stmt_prepare($stmt, $sql)){
                    ...
                    $typeUser = "eleman";
                    mysqli_stmt_bind_param($stmt , "sssssss", substr($fileDestination, 3), $name, $surname, $username, $email, $hashedPwd, $typeUser);
                    mysqli_stmt_execute($stmt);
